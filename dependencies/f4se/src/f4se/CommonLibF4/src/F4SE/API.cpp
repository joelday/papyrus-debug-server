...
#include "f4se_common/BranchTrampoline.h"  // g_localTrampoline, g_branchTrampoline
...
namespace F4SE
{
	namespace
	{
		...
		F4SEDelayFunctorManager*		g_delayFunctorManager = 0;
		F4SEObjectRegistry*				g_objectRegistry = 0;
		F4SEPersistentObjectStorage*	g_persistentObjectStorage = 0;
	...
	bool Init(const LoadInterface* a_f4se)
	{
		...
		if (!g_apiInit) {
			...
			g_releaseIndex = a_f4se->GetReleaseIndex();

			//g_scaleformInterface = static_cast<decltype(g_scaleformInterface)>(a_f4se->QueryInterface(InterfaceID::kScaleform));
			//if (!g_scaleformInterface) {
			//	_ERROR("Failed to query scaleform interface!\n");
			//	return false;
			//}

			//g_papyrusInterface = static_cast<decltype(g_papyrusInterface)>(a_f4se->QueryInterface(InterfaceID::kPapyrus));
			//if (!g_papyrusInterface) {
			//	_ERROR("Failed to query papyrus interface!\n");
			//	return false;
			//}

			//g_serializationInterface = static_cast<decltype(g_serializationInterface)>(a_f4se->QueryInterface(InterfaceID::kSerialization));
			//if (!g_serializationInterface) {
			//	_ERROR("Failed to query serialization interface!\n");
			//	return false;
			//}

			g_taskInterface = static_cast<decltype(g_taskInterface)>(a_f4se->QueryInterface(InterfaceID::kTask));
			if (!g_taskInterface) {
				_ERROR("Failed to query task interface!\n");
				return false;
			}

			g_messagingInterface = static_cast<decltype(g_messagingInterface)>(a_f4se->QueryInterface(InterfaceID::kMessaging));
			...
			g_objectInterface = static_cast<decltype(g_objectInterface)>(a_f4se->QueryInterface(InterfaceID::kObject));
