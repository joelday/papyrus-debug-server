...
#include "f4se/GameRTTI.h"  // RTTI_BSScript__ObjectTypeInfo
...
namespace RE
{
	namespace BSScript
	{
		class ObjectTypeInfo : public IComplexType
		{
		public:
			inline static const void* RTTI = RTTI_BSScript__ObjectTypeInfo;
			...
			struct ScriptVariable
			{
				...
				VMTypeID		type;	// 08
				UInt64	unk10;			// From F4SE: 10 - Not sure what this is, maybe a hash?
			};
			STATIC_ASSERT(sizeof(ScriptVariable) == 0x18);
			...
			STATIC_ASSERT(sizeof(ScriptPropertyGroup) == 0x38);
			...
			VMTypeID GetTypeID() const override;

			UInt32		GetNumGroups() const;
			ScriptPropertyGroup* GetGroupIter();
			const ScriptPropertyGroup* GetGroupIter() const;
			...
			BSTSmartPointer<ObjectTypeInfo>	parent;			// 10
			BSFixedString			unk18;					// 18
			ScriptPropertyGroup**	groups;			// 20
			UInt64					numGroups;		// 28 // TODO: Verify
			UInt64					unk30;					// 30
			
			bool					inherited : 1;			// 20 - 0
			bool					removed : 1;			// 20 - 1 - false -> data == Function*, true -> data == normal progression
			bool					unk40_2 : 1;
			bool					numFlags1 : 1;			// 20 - 3
			bool					numFlags2 : 1;			// 20 - 4
			bool					numFlags3 : 1;			// 20 - 5
			bool					numFlags4 : 1;			// 20 - 6
			bool					numFlags5 : 1;			// 20 - 7
		...
		STATIC_ASSERT(offsetof(ObjectTypeInfo, data) == 0x50);
	}
}
