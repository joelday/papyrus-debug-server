...
{
	DebugServer::DebugServer(F4SETaskInterface* tasks) :
		m_session(NULL), m_tasks(tasks)
	{
    ...
    void DebugServer::HandleOpen(websocketpp::connection_hdl hdl)
    {
		...
		m_session = new DebugServerSession(std::bind(&DebugServer::Send, this, ::_1), m_tasks);
    }
    ...
    UInt32 DebugServer::ListenInternal()
    {
        try
		{
			...
			m_server.set_max_message_size(1024 * 1024 * 5);
			m_server.set_max_http_body_size(1024 * 1024 * 5);
